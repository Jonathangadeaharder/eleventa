...
class DocumentPdfGenerator:
    ...
    def generate_invoice_pdf(
        ...
        try:
            ...
            if (
                "customer" not in invoice_data
                and "customer_details" not in invoice_data
            ):
            ...
            self.logger.error(
                f"Error generating invoice PDF: {e}"
            )  # Changed from print
            return False
    ...
    def _create_invoice_header(self, invoice_data: Dict[str, Any]) -> List:
        ...
        # Store name and logo
        if self.store_info.get("logo_path") and os.path.exists(
            self.store_info["logo_path"]
        ):
            try:
                logo = Image(
                    self.store_info["logo_path"], width=1.5 * inch, height=0.75 * inch
                ...
                )  # Changed from print, non-critical
        ...
        date_str = (
            invoice_date_obj.strftime("%d/%m/%Y")
            if isinstance(invoice_date_obj, datetime)
            else str(invoice_date_obj)
        )
    ...
    def _create_invoice_customer_section(self, invoice_data: Dict[str, Any]) -> List:
        ...
        customer_section_data = [
            [
                Paragraph("<b>Cliente:</b>", self.styles["DocInfo"]),
                Paragraph(customer_details.get("name", ""), self.styles["DocInfo"]),
            ],
            [
                Paragraph("<b>CUIT:</b>", self.styles["DocInfo"]),
                Paragraph(customer_details.get("cuit", ""), self.styles["DocInfo"]),
            ],
            [
                Paragraph("<b>Domicilio:</b>", self.styles["DocInfo"]),
                Paragraph(customer_details.get("address", ""), self.styles["DocInfo"]),
            ],
            [
                Paragraph("<b>Condición frente al IVA:</b>", self.styles["DocInfo"]),
                Paragraph(
                    customer_details.get("iva_condition", "Consumidor Final"),
                    self.styles["DocInfo"],
                ),
            ],
        ]
        ...
        customer_table = Table(
            customer_section_data, colWidths=[2.5 * cm, None]
        )  # Adjust first col width
    ...
    def _create_invoice_items_table(self, sale_items: List[Dict[str, Any]]) -> List:
        ...
        for item in sale_items:
            ...
            row = [
                Paragraph(
                    str(item.get("product_code", "")), self.styles["DocInfo"]
                ),  # Was item.get('code', '')
                Paragraph(
                    str(item.get("product_description", "")), self.styles["DocInfo"]
                ),  # Was item.get('description', '')
        ...
        items_table = Table(
            data, colWidths=[1.5 * cm, 6 * cm, 2 * cm, 2.5 * cm, 2.5 * cm]
        )  # Adjusted widths
        ...
        if invoice_data.get("invoice_type") == "A":
            totals_data = [
                [
                    ...
                    "",
                    "",
                    Paragraph("Subtotal:", self.styles["BoldRightAlign"]),
                    Paragraph(f"${subtotal:.2f}", self.styles["RightAlign"]),
                ],
                [
                    "",
                    "",
                    "",
                    Paragraph("IVA (21%):", self.styles["BoldRightAlign"]),
                    Paragraph(f"${iva_amount:.2f}", self.styles["RightAlign"]),
                ],
                [
                    "",
                    "",
                    "",
                    Paragraph("TOTAL:", self.styles["BoldRightAlign"]),
                    Paragraph(f"${total:.2f}", self.styles["BoldRightAlign"]),
                ],
            ]
    ...
    def _format_item_row_receipt(self, item: Dict[str, Any]) -> List[Any]:
        ...
        product_description = str(item.get("product_description", "N/A"))[
            :30
        ]  # Truncate
        ...
        return [
            ...
            Paragraph(
                self._format_currency_receipt(unit_price), self.styles["RightAlign"]
            ),
            Paragraph(
                self._format_currency_receipt(subtotal), self.styles["RightAlign"]
            ),
        ]
    ...
    def generate_receipt(self, sale_data: Dict[str, Any], filename: str) -> bool:
        ...
        try:
            ...
            elements.append(
                Paragraph(
                    self.store_info.get("name", "Store Name"), self.styles["DocTitle"]
                )
            )  # Use DocTitle
            elements.append(
                Paragraph(
                    f"Dirección: {self.store_info.get('address', '')}",
                    self.styles["DocInfo"],
                )
            )
            elements.append(
                Paragraph(
                    f"Teléfono: {self.store_info.get('phone', '')}",
                    self.styles["DocInfo"],
                )
            )
            if self.store_info.get("cuit"):
                elements.append(
                    Paragraph(
                        f"CUIT: {self.store_info['cuit']}", self.styles["DocInfo"]
                    )
                )
            ...
            elements.append(
                Paragraph(
                    ...
                    self.styles.get("ReceiptTitle", self.styles["Heading2"]),
                )
            )
            elements.append(Spacer(1, 0.1 * inch))
            ...
            elements.append(
                Paragraph(
                    f"Venta #: {sale_data.get('id', 'N/A')}", self.styles["DocInfo"]
                )
            )
            elements.append(
                Paragraph(
                    f"Fecha: {self._format_sale_date_receipt(sale_data.get('timestamp'))}",
                    self.styles["DocInfo"],
                )
            )
            if sale_data.get(
                "user_name"
            ):  # Assuming user_name might be added to sale_data by the service
                elements.append(
                    Paragraph(
                        f"Atendido por: {sale_data['user_name']}",
                        self.styles["DocInfo"],
                    )
                )
            elements.append(
                Paragraph(
                    f"Forma de pago: {sale_data.get('payment_type', 'N/A')}",
                    self.styles["DocInfo"],
                )
            )
            if sale_data.get("customer_name"):  # Assuming customer_name might be added
                elements.append(
                    Paragraph(
                        f"Cliente: {sale_data['customer_name']}", self.styles["DocInfo"]
                    )
                )
            ...
            table_data = [
                [Paragraph(h, self.styles["ItemsTableHeader"]) for h in headers]
            ]

            sale_items = sale_data.get("items", [])
            if not isinstance(sale_items, list):
                self.logger.warning(
                    f"Warning: sale_items is not a list in sale_data: {sale_items}. Skipping items table."
                )  # Changed from print
            ...
            for item_dict in sale_items:
                ...
                if not isinstance(item_dict, dict):
                    self.logger.warning(
                        f"Warning: item in sale_items is not a dict: {item_dict}. Skipping this item."
                    )  # Changed from print
                ...
                TableStyle(
                    [
                        ...
                        (
                            ...
                            (-1, 0),
                            "Helvetica-Bold",
                        ),  # Already in ItemsTableHeader style
            ...
            # Total
            total_amount_val = sale_data.get(
                "total", Decimal("0")
            )  # Prefer 'total' from Sale model
            if (
                not total_amount_val and sale_items
            ):  # Fallback to sum items if total is not directly available
                calculated_total = sum(
                    Decimal(str(it.get("subtotal", "0")))
            ...
            elements.append(
                Paragraph("¡Gracias por su compra!", self.styles["DocInfo"])
            )
            ...
            self.logger.error(
                f"Error generating receipt PDF: {e}"
            )  # Changed from print
            return False

    # --- Presupuesto Generation Methods (Adapted from SaleService) ---
    def _create_presupuesto_header_footer(
        self,
        elements: List,
        presupuesto_id: Optional[str],
        ...
        elements.append(
            Paragraph(
                self.store_info.get("name", "SU NEGOCIO"), self.styles["DocTitle"]
            )
        )
        elements.append(
            Paragraph(
        ...
        elements.append(
            Paragraph(
                f"Teléfono: {self.store_info.get('phone', '')}", self.styles["DocInfo"]
            )
        )
        if self.store_info.get("cuit"):
            elements.append(
                Paragraph(
                    f"CUIT: {self.store_info.get('cuit', '')}", self.styles["DocInfo"]
                )
            )
        elements.append(Spacer(1, 0.2 * inch))
        ...
        if presupuesto_id:
            elements.append(
                Paragraph(f"Presupuesto N°: {presupuesto_id}", self.styles["DocInfo"])
            )
        elements.append(
            Paragraph(
        ...
        if user_name:
            elements.append(
                Paragraph(f"Atendido por: {user_name}", self.styles["DocInfo"])
            )
        if customer_name:
            elements.append(
                Paragraph(f"Cliente: {customer_name}", self.styles["DocInfo"])
            )
        elements.append(Spacer(1, 0.2 * inch))
    ...
    def generate_presupuesto(
        ...
        try:
            ...
            self._create_presupuesto_header_footer(
                elements, presupuesto_id, user_name, customer_name
            )
            ...
            for item_dict in items_data:
                ...
                table_items_data.append(
                    [
                        ...
                        Paragraph(f"{qty:.2f}", self.styles["RightAlign"]),
                        Paragraph(
                            self._format_currency_receipt(unit_price_val),
                            self.styles["RightAlign"],
                        ),
                        Paragraph(
                            self._format_currency_receipt(subtotal_val),
                            self.styles["RightAlign"],
                        ),
                    ]
                )
            ...
            elements.append(
                Paragraph(
                    f"TOTAL PRESUPUESTADO: {self._format_currency_receipt(total_amount_decimal)}",
                    self.styles.get("Total", self.styles["BoldRightAlign"]),
                )
            )
            elements.append(Spacer(1, 0.3 * inch))
            elements.append(
                Paragraph(
                    "Precios sujetos a cambio sin previo aviso.", self.styles["DocInfo"]
                )
            )
            elements.append(
                Paragraph(
                    "Este presupuesto tiene una validez de 7 días.",
                    self.styles["DocInfo"],
                )
            )
            ...
            self.logger.error(
                f"Error generating presupuesto PDF: {e}"
            )  # Changed from print
            return False
    ...
    def generate_receipt_from_sale(self, sale, filename: str) -> bool:
        ...
        try:
            ...
            story.append(
                Paragraph(
                    f"Sale ID: {sale.id if sale.id is not None else 'N/A'}",
                    self.styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"Date: {sale.timestamp.strftime('%Y-%m-%d %H:%M:%S')}",
                    self.styles["Normal"],
                )
            )
            if sale.payment_type:
                story.append(
                    Paragraph(
                        f"Payment Type: {sale.payment_type.value}",
                        self.styles["Normal"],
                    )
                )
            if sale.customer_id:
                story.append(
                    Paragraph(f"Customer ID: {sale.customer_id}", self.styles["Normal"])
                )
            if sale.user_id:
                story.append(
                    Paragraph(f"User ID: {sale.user_id}", self.styles["Normal"])
                )
            story.append(Spacer(1, 0.2 * inch))
            ...
            data.append(
                [
                    ...
                    Paragraph(f"<b>${sale.total:.2f}</b>", self.styles["Normal"]),
                ]
            )

            table = Table(
                data, colWidths=[0.7 * inch, 2.5 * inch, 0.5 * inch, 1 * inch, 1 * inch]
            )
            table.setStyle(
                TableStyle(
                    [
            ...
            # Footer Message
            story.append(
                Paragraph("Thank you for your purchase!", self.styles["Normal"])
            )

            doc.build(story)
            self.logger.info(
                f"Receipt {filename} generated successfully for sale ID {sale.id}"
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Error generating receipt PDF for sale ID {sale.id}: {e}"
            )
            return False

    def generate_presupuesto_content(
        self,
        filename: str,
        items: List[Any],
        total_amount: Decimal,
        ...
        try:
            ...
            if presupuesto_id:
                story.append(
                    Paragraph(f"<b>Presupuesto ID:</b> {presupuesto_id}", info_style)
                )
            story.append(
                Paragraph(
            ...
            for item in items:
                try:
                    ...
                    data.append(
                        [
                            (
                                ...
                                if isinstance(item, dict)
                                else getattr(item, "product_code", "N/A")
                            ),
                            (
                                item.get("product_description")
                                if isinstance(item, dict)
                                else getattr(item, "product_description", "N/A")
                            ),
                            f"{quantity_val:.2f}",
                            f"${unit_price_val:.2f}",
                            f"${subtotal_val:.2f}",
                    ...
                    self.logger.error(
                        f"Error processing item for PDF: {item}. Error: {e}"
                    )
            ...
            table = Table(
                data, colWidths=[0.8 * inch, 3 * inch, 0.6 * inch, 1 * inch, 1.1 * inch]
            )
            table.setStyle(
                TableStyle(
                    [
            ...
            story.append(
                Paragraph("Este documento no es un comprobante fiscal.", footer_style)
            )
...
if __name__ == "__main__":
    ...
    sample_sale_items = [
        {
            "product_code": "P001",
            "product_description": "Producto Alfa",
            "quantity": Decimal("2.00"),
            "unit_price": Decimal("300.00"),
        },
        {
            "product_code": "P002",
            "product_description": "Producto Beta Largo Nombre",
    ...
    if generator.generate_invoice_pdf(
        sample_invoice_data, sample_sale_items, invoice_filename
    ):
        print(f"Sample invoice generated: {invoice_filename}")
    else:
        print("Failed to generate sample invoice.")

    # Sample Data for Presupuesto
    sample_presupuesto_items = [
        {
            "product_description": "Servicio de Consultoría",
            "quantity": Decimal("10.0"),
            "unit_price": Decimal("50.00"),
        },
        {
            "product_description": "Licencia Software XYZ (Anual)",
            "quantity": Decimal("1.0"),
            "unit_price": Decimal("250.00"),
        },
    ]
    presupuesto_total = sum(
        it["quantity"] * it["unit_price"] for it in sample_presupuesto_items
    )
    ...
    if generator.generate_presupuesto(
        ...
        print("Failed to generate sample presupuesto.")
    ...
    sample_sale_data_receipt = {
        ...
        "items": [
            ...
            {
                "product_code": "P002",
                "product_description": "Producto Beta",
                "quantity": Decimal("1.00"),
                "unit_price": Decimal("400.00"),
    ...
    if generator.generate_receipt(
        sale_data=sample_sale_data_receipt, filename=receipt_filename
    ):
        print(f"Sample receipt generated: {receipt_filename}")
    else:
        print("Failed to generate sample receipt.")
