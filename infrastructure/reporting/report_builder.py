...
class ReportBuilder:
    ...
    def generate_report_pdf(
        ...
        """
        logging.debug(
            f"ReportBuilder.generate_report_pdf called with filename: {filename}, is_abs: {os.path.isabs(filename)}"
        )
        try:
            ...
            output_dir = os.path.dirname(
                abs_filename
            )  # If filename is absolute, output_dir will be too.
            if not os.path.isabs(
                output_dir
            ):  # This check might become redundant but kept for safety
                logging.warning(
                    f"ReportBuilder received filename '{filename}' (abs: '{abs_filename}') resulting in relative output_dir '{output_dir}'. This is unexpected."
                )
    ...
    def _create_header(self, report_title: str) -> List:
        ...
        store_phone = Paragraph(
            f"Tel: {self.store_info.get('phone', '')}", styles["Normal"]
        )
    ...
    def _create_sales_by_period_report(self, report_data: Dict[str, Any]) -> List:
        ...
        # Sales table header
        elements.append(
            Paragraph("<b>Detalle de ventas por período:</b>", styles["Heading3"])
        )
        ...
        if len(data) > 1:
            ...
            elements.append(
                Paragraph(
                    "No hay datos disponibles para este período.", styles["Normal"]
                )
            )
    ...
    def _create_sales_by_department_report(self, report_data: Dict[str, Any]) -> List:
        ...
        elements.append(
            Paragraph("<b>Ventas por departamento:</b>", styles["Heading3"])
        )

        # Sales by department table data
        data = [
            ["Departamento", "Total Ventas", "Cantidad de Artículos", "% del Total"]
        ]
    ...
    def _create_profit_analysis_report(self, report_data: Dict[str, Any]) -> List:
        ...
        if "department_profit" in report_data:
            elements.append(
                Paragraph("<b>Desglose por departamento:</b>", styles["Heading3"])
            )
    ...
    def _create_cash_drawer_report(self, report_data: Dict[str, Any]) -> List:
        ...
        if entries:
            elements.append(
                Paragraph("<b>Movimientos de Caja:</b>", styles["Heading3"])
            )
            ...
            for entry in entries:
                ...
                type_map = {"START": "Apertura", "IN": "Ingreso", "OUT": "Retiro"}
            ...
            elements.append(
                Paragraph("No hay movimientos registrados.", styles["Normal"])
            )
    ...
    def _create_corte_report(self, report_data: Dict[str, Any]) -> List:
        ...
        elements.append(
            Paragraph("<b>Ventas por Forma de Pago</b>", styles["Heading3"])
        )
        ...
        cash_data = [
            ...
            [
                ...
                f"${report_data.get('expected_cash', 0):.2f}",
            ],
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (
                            colors.red
                            if report_data.get("difference", 0) < 0
                            else (
                                colors.green
                                if report_data.get("difference", 0) > 0
                                else colors.black
                    ...
                    ),  # Color difference based on value
                ]
            )
        )
        ...
        if report_data.get("cash_in_entries"):
            elements.append(
                Paragraph("<b>Entradas de Efectivo</b>", styles["Heading3"])
            )
            ...
            if len(cash_in_data) > 1:
                ...
                elements.append(
                    Paragraph(
                        "No hay entradas de efectivo registradas.", styles["Normal"]
                    )
                )
        ...
        if report_data.get("cash_out_entries"):
            ...
            if len(cash_out_data) > 1:
                ...
                elements.append(
                    Paragraph(
                        "No hay salidas de efectivo registradas.", styles["Normal"]
                    )
                )
