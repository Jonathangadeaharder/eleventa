...
class ModelMapper:
    ...
    def product_orm_to_domain(prod_orm: "ProductOrm") -> Optional[Product]:
        ...
        department_model = (
            ModelMapper.department_orm_to_domain(prod_orm.department)
            if prod_orm.department
            else None
        )
    ...
    def sale_orm_to_domain(sale_orm: "SaleOrm") -> Optional[Sale]:
        ...
        # Map related items
        items_model = (
            [ModelMapper.sale_item_orm_to_domain(item) for item in sale_orm.items]
            if sale_orm.items
            else []
        )
    ...
    def customer_orm_to_domain(cust_orm: "CustomerOrm") -> Optional[Customer]:
        ...
        return Customer(
            ...
            credit_limit=(
                Decimal(str(cust_orm.credit_limit))
                if cust_orm.credit_limit is not None
                else Decimal("0.0")
            ),
            credit_balance=(
                Decimal(str(cust_orm.credit_balance))
                if cust_orm.credit_balance is not None
                else Decimal("0.0")
            ),
    ...
    @staticmethod
    def inventory_movement_orm_to_domain(
        move_orm: "InventoryMovementOrm",
    ...
    @staticmethod
    def credit_payment_orm_to_domain(
    ...
    def invoice_orm_to_domain(invoice_orm: "InvoiceOrm") -> Optional[Invoice]:
        ...
        if invoice_orm.customer_details:
            try:
                ...
                logging.warning(
                    f"Could not decode customer_details JSON for invoice {invoice_orm.id}"
                )
                customer_details_dict = {}
    ...
    @staticmethod
    def invoice_domain_to_orm(
        invoice: Invoice, invoice_orm: Optional["InvoiceOrm"] = None
    ) -> "InvoiceOrm":
        ...
        try:
            ...
            logging.warning(
                f"Could not serialize customer_details for invoice related to sale {invoice.sale_id}"
            )
