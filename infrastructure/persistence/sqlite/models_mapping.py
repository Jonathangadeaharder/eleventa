...
class ProductOrm(Base):
    ...
    quantity_in_stock = Column(
        Numeric(10, 3), nullable=False, default=0.0
    )  # Allow 3 decimal places for quantity
    min_stock = Column(
        Numeric(10, 3), nullable=True, default=0.0
    )  # Allow 3 decimal places for quantity
    max_stock = Column(
...
class InventoryMovementOrm(Base):
    ...
    )  # Positive for addition, negative for removal, 3 decimal places
    description = Column(String, nullable=True)
    related_id = Column(
        Integer, nullable=True, index=True
    )  # e.g., Sale ID, Purchase ID

    # Relationship: Many-to-One (Many Movements belong to One Product)
    product = relationship(
        "ProductOrm"
    )  # No back_populates needed if ProductOrm doesn't track movements directly
...
class SaleOrm(Base):
    ...
    customer_id = Column(
        SQLiteUUID, ForeignKey("customers.id"), nullable=True, index=True
    )
...
class SaleItemOrm(Base):
    ...
    product = relationship(
        "ProductOrm"
    )  # No back_populates unless Product needs SaleItems
...
class CustomerOrm(Base):
    ...
    credit_payments = relationship(
        "CreditPaymentOrm", back_populates="customer"
    )  # Added CreditPayment relationship
...
class CreditPaymentOrm(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    customer_id = Column(
        SQLiteUUID, ForeignKey("customers.id"), nullable=False, index=True
    )
...
class InvoiceOrm(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    sale_id = Column(
        Integer, ForeignKey("sales.id"), nullable=False, unique=True, index=True
    ...
    # Relationships
    sale = relationship(
        "SaleOrm", backref="invoice"
    )  # One-to-one relationship with sale
...
class CashDrawerEntryOrm(Base):
    ...
    entry_type = Column(
        String, nullable=False, index=True
    )  # Will be mapped to CashDrawerEntryType enum
    amount = Column(Numeric(12, 2), nullable=False)
...
def ensure_all_models_mapped():
    ...
    for model in model_classes:
        ...
        if table_name not in tables_in_metadata:
            print(
                f"  - ERROR: Model {model.__name__} table '{table_name}' not found in Base.metadata."
            )
            all_mapped = False
            missing_tables.append(table_name)
        else:
            print(
                f"  - Model {model.__name__} correctly mapped to table '{table_name}'"
            )
    ...
    if not all_mapped:
        ...
        print("Attempting to force registration by accessing __table__...")
        try:
            for model in model_classes:
            ...
            tables_in_metadata = list(Base.metadata.tables.keys())
            print(
                f"Tables in Base.metadata after forced registration: {tables_in_metadata}"
            )
            remaining_missing = [
                m.__tablename__
                for m in model_classes
                if hasattr(m, "__tablename__")
                and m.__tablename__ not in tables_in_metadata
            ]
            if remaining_missing:
                raise RuntimeError(
                    f"SQLAlchemy mapping failed. Missing tables in metadata after attempt: {remaining_missing}"
                )
            else:
    ...
    if not all_mapped:
        ...
        print("Please ensure:")
        print(
            "  1. All ORM classes in models_mapping.py inherit from the 'Base' defined in database.py."
        )
        print(
            "  2. The 'infrastructure.persistence.sqlite.database' module correctly initializes 'Base'."
        )
        print("  3. Imports are handled correctly to avoid circular dependencies.")
        raise RuntimeError(
            f"SQLAlchemy mapping verification failed. Missing tables: {missing_tables}"
        )

    print(
        f"Successfully verified {len(model_classes)} models mapped to {len(tables_in_metadata)} tables in Base.metadata."
    )
    return True
