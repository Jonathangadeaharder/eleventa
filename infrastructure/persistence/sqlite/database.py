...
def init_db():
    ...
    from infrastructure.persistence.sqlite.table_deps import (
    ...
    print(
        f"Creating/updating database tables defined in Base metadata ({len(Base.metadata.tables)} tables)..."
    )
    try:
...
def create_all_tables(engine_instance):
    ...
    print(
        f"Creating all database tables with {len(registered_tables)} tables registered: {registered_tables}"
    )  # Log registered table names
