...
from core.value_objects import Money, Email, Address, Percentage, ProductCode, PhoneNumber, TaxId
from core.specifications.base import Specification, AndSpecification, OrSpecification, NotSpecification
from core.aggregates.examples import Order, ShoppingCart, Customer
from core.aggregates.base import DomainError, AggregateRoot, Entity
from core.aggregates.repository import InMemoryAggregateRepository, InMemoryEventPublisher
...
run_test("Money: Add same currency", lambda: Money(Decimal('10'), 'USD').add(Money(Decimal('5'), 'USD')))
run_test("Money: Subtract", lambda: Money(Decimal('10'), 'USD').subtract(Money(Decimal('5'), 'USD')))
...
run_test("Money: Convert currency", lambda: Money(Decimal('100'), 'USD').convert_to('EUR', Decimal('0.85')))
run_test("Money: Is zero", lambda: Money.zero('USD').is_zero())
run_test("Money: Is positive", lambda: Money(Decimal('1'), 'USD').is_positive())
run_test("Money: Comparison", lambda: Money(Decimal('10'), 'USD') < Money(Decimal('20'), 'USD'))
...
run_test("Email: Check domain", lambda: Email('john@example.com').is_from_domain('example.com'))
run_test("Email: Obfuscate", lambda: Email('john@example.com').obfuscate())

# Address tests
run_test("Address: Create", lambda: Address('123 Main St', 'Springfield', '62701', 'USA'))
run_test("Address: To string", lambda: str(Address('123 Main St', 'Springfield', '62701', 'USA')))
run_test("Address: Multiline", lambda: Address('123 Main St', 'Springfield', '62701', 'USA').to_multiline())
run_test("Address: Is in country", lambda: Address('123 Main St', 'Springfield', '62701', 'USA').is_in_country('USA'))
run_test("Address: Create simple", lambda: Address.create_simple('123 Main St', 'Springfield', '62701'))
...
run_test("Percentage: Subtract from", lambda: Percentage(Decimal('10')).subtract_from(Decimal('100')))
run_test("Percentage: Add percentages", lambda: Percentage(Decimal('5')).add(Percentage(Decimal('3'))))
...
run_test("Order: Add item", lambda: Order(uuid4()).add_item(uuid4(), "Widget", 1, Money(Decimal('10'), 'USD')))
...
for filepath in files:
    if any(pattern in filepath for pattern in ['value_objects', 'specifications', 'aggregates']):
