...
from ui.utils import show_error_message  # Assuming utility functions
from ui.dialogs.add_inventory_dialog import AddInventoryDialog  # Import the dialog
from ui.dialogs.adjust_inventory_dialog import (
    AdjustInventoryDialog,
)  # Import the new dialog
from ui.dialogs.import_export_dialog import (
    ImportExportDialog,
)  # Import the import/export dialog
from core.models.product import Product  # Import Product model
from ui.widgets.filter_dropdowns import (
    FilterBoxWidget,
    FilterDropdown,
)
...
class InventoryView(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.add_button = QPushButton(
            QIcon(":/icons/icons/new.png"), " Agregar Cantidad"
        )
        self.adjust_button = QPushButton(
            QIcon(":/icons/icons/edit.png"), " Ajustar Stock"
        )
        self.import_export_button = QPushButton(
            QIcon(":/icons/icons/import.png"), " Importar/Exportar"
        )
        self.report_button = QPushButton(
            QIcon(":/icons/icons/reports.png"), " Reporte de Inventario"
        )
        self.low_stock_button = QPushButton(
            QIcon(":/icons/icons/inventory.png"), " Productos Bajos"
        )
        self.movements_button = QPushButton(
            QIcon(":/icons/icons/inventory.png"), " Movimientos"
        )
        self.kardex_button = QPushButton(
            QIcon(":/icons/icons/inventory.png"), " Kardex"
        )
        ...
        toolbar_layout.addSpacerItem(
            QSpacerItem(20, 20, QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Minimum)
        )
        ...
        summary_layout.addSpacerItem(
            QSpacerItem(40, 20, QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Minimum)
        )
    ...
    def refresh_inventory_report(self):
        ...
        try:
            ...
            print(
                f"Inventory report refreshed with {len(inventory_products)} items."
            )  # Debug
            ...
            show_error_message(
                ...
                f"No se pudo cargar el reporte de inventario: {e}",
            )
    ...
    def _update_report_totals(self, products):
        ...
        total_cost = sum(
            p.quantity_in_stock * p.cost_price
            for p in products
            if p.quantity_in_stock is not None and p.cost_price is not None
        )
        total_sell = sum(
            p.quantity_in_stock * p.sell_price
            for p in products
            if p.quantity_in_stock is not None and p.sell_price is not None
        )
    ...
    def refresh_low_stock_report(self):
        ...
        try:
            # Get the selected department ID (if any)
            self.department_filter.get_selected_value()
            ...
            print(
                f"Low stock report refreshed with {len(low_stock_products)} items."
            )  # Debug
        except Exception as e:
            show_error_message(
                self,
                "Error al Cargar Productos Bajos",
                f"No se pudo cargar el reporte de bajo stock: {e}",
            )
    ...
    def add_inventory_item(self):
        ...
        dialog = AddInventoryDialog(
            self.inventory_service, selected_product, self.current_user, self
        )
    ...
    def open_import_export_dialog(self):
        ...
        if dialog.exec() == ImportExportDialog.DialogCode.Accepted:
            ...
            print(
                "[InventoryView] Import/Export dialog accepted. Refreshing inventory view."
            )
            current_index = self.tab_widget.currentIndex()
            if current_index == 0:
                self.refresh_inventory_report()
...
if __name__ == "__main__":
    ...
    class MockProductService:
        def get_all_products(self, department_id=None):
            # Returns empty list or can simulate products if needed
            return []
