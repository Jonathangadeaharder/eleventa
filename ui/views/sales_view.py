...
class SalesView(QWidget):
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"Blank QPixmap created: {blank_pixmap}, isNull: {blank_pixmap.isNull()}"
            )
    ...
    def _init_ui(self):
        ...
        self.product_combo.setFocusPolicy(
            Qt.FocusPolicy.StrongFocus
        )  # Explicit focus policy
        self.product_combo.lineEdit().setFocusPolicy(
            Qt.FocusPolicy.StrongFocus
        )  # Explicit focus policy for lineEdit
        self.product_combo.view().setVerticalScrollBarPolicy(
        ...
        self.add_button.setIcon(
            QIcon(":/icons/icons/new.png")
        )  # Reverted to original resource path
        style_secondary_button(self.add_button)

        self.remove_item_button = QPushButton(
            "Quitar Artículo"
        )  # Define remove_item_button here
        ...
        self.select_customer_button.setIcon(
            QIcon(":/icons/icons/customers.png")
        ...
        self.presupuesto_button.setIcon(
            QIcon(":/icons/icons/print.png")
        )  # Placeholder icon, consider a specific one
        style_secondary_button(self.presupuesto_button)

        self.cancel_button = QPushButton("Cancelar Venta (F3)")
        self.cancel_button.setIcon(
            QIcon(":/icons/icons/cancel.png")
        )  # Reverted to original
        ...
        self.finalize_button.setStyleSheet(
            self.finalize_button.styleSheet() + "font-size: 14px;"
        )
        ...
        action_buttons_layout.addWidget(
            self.presupuesto_button
        )  # Add Presupuesto button here
        action_buttons_layout.addWidget(self.cancel_button)
        action_buttons_layout.addWidget(
            self.invoice_button
        )  # Add invoice button to layout
        action_buttons_layout.addStretch(1)
    ...
    def _connect_signals(self):
        ...
        self.table_view.doubleClicked.connect(
            self._edit_selected_item_quantity
        )  # Placeholder
        ...
        self.sale_item_model.dataChanged.connect(
            self.update_total
        )  # Connect dataChanged
        self.sale_item_model.modelReset.connect(self.update_total)  # Connect modelReset
        self.sale_item_model.rowsRemoved.connect(
            self.update_total
        )  # Connect rowsRemoved
        self.invoice_button.clicked.connect(self.generate_invoice_from_sale)
        self.presupuesto_button.clicked.connect(
            self._generate_presupuesto_pdf
        )  # Connect new button

        # Connect search signal for product combo
        self.product_combo.lineEdit().textEdited.connect(
            self._search_and_suggest_products
        )
        self.product_combo.activated.connect(
            self._product_selected_from_combo
        )  # Handle selection
    ...
    def _product_selected_from_combo(self, index: int):
        ...
        if (
            index >= 0
        ):  # Ensure a valid item was selected (index -1 means no selection or text edited)
    ...
    def _load_customers(self):
        ...
        try:
            ...
            show_error_message(
                self, "Error", f"No se pudieron cargar los clientes: {e}"
            )
            self._customers = []
    ...
    def _get_selected_customer_id(self) -> Optional[int]:
        ...
        elif hasattr(self, "customer_combo") and hasattr(
        ...
        if self._suggestion_model.stringList():  # Check if model has strings
            ...
            if popup:
                ...
                def move_popup_action():
                    ...
                    global_y = combo_box.mapToGlobal(
                        QPoint(0, combo_box.height() + 4)
                    ).y()
        ...
        if selected_indexes:
            ...
            if item:
                ...
                self.sale_item_model.dataChanged.emit(
                    ...
                    self.sale_item_model.index(
                        row, self.sale_item_model.columnCount() - 1
                    ),
                )
                self.update_total()
                return
            ...
            if selected_data and isinstance(selected_data, Product):
                ...
                if not code_or_name:
                    show_error_message(
                        self,
                        "Producto Requerido",
                        "Por favor, ingrese el código o nombre de un producto para agregar.",
                    )
                    self.product_combo.setFocus()
                    return
                ...
                try:
                    ...
                    product_by_code = self.product_service.get_product_by_code(
                        code_or_name
                    )
                    if product_by_code:
                        ...
                        possible_products = self.product_service.find_product(
                            code_or_name
                        )
                    ...
                    show_error_message(
                        self, "Error de Búsqueda", f"Error al buscar producto: {e}"
                    )
                    self.product_combo.setFocus()
                    return
        ...
        if not product_to_add:
            ...
            if texto_ingresado:
                ...
                show_error_message(
                    self,
                    "Producto Requerido",
                    "Por favor, seleccione un producto para agregar.",
                )
        ...
        try:
            ...
            if product_to_add.sell_price is None:
                show_error_message(
                    self,
                    "Error de Precio",
                    f"El producto '{product_to_add.code}' no tiene un precio de venta definido.",
                )
                self.product_combo.lineEdit().selectAll()
                self.product_combo.setFocus()
                return
            ...
            print(
                f"Adding product: {product_to_add.code}, price: {product_to_add.sell_price}, as Decimal: {unit_price}"
            )
            ...
            show_error_message(
                self, "Error", f"Ocurrió un error al agregar el producto: {e}"
            )
            self.product_combo.setFocus()
    ...
    def open_pdf_file(self, file_path):
        ...
        try:
            ...
            show_error_message(
                self, "Error al Abrir PDF", f"No se pudo abrir el archivo PDF: {e}"
            )
            return False
    ...
    def print_receipt(self, sale_id):
        ...
        try:
            ...
            show_error_message(
                self, "Error al Generar Recibo", f"No se pudo generar el recibo: {e}"
            )
    ...
    def finalize_current_sale(self):
        ...
        )  # Allow credit only if a customer is selected
        ...
        if (
            not payment_method
        ):  # Should not happen if validation in dialog is correct, but check anyway
            show_error_message(
                self, "Error", "No se seleccionó un método de pago válido."
            )
            return
        ...
        try:
            ...
            show_info_message(
                self,
                "Venta Finalizada",
                f"Venta #{created_sale.id} registrada exitosamente.",
            )
            ...
            show_error_message(
                self, "Error al Finalizar", f"No se pudo registrar la venta: {e}"
            )
                ...
                try:
                    if not hasattr(self.sale_service, "assign_customer_to_sale"):
                        show_error_message(
                            ...
                            "SaleService no tiene el método 'assign_customer_to_sale'.",
                        )
                        return False
                    self.sale_service.assign_customer_to_sale(
                        sale_id, selected_customer.id
                    )
                    show_info_message(
                        self,
                        "Cliente Asignado",
                        "Cliente asignado correctamente a la venta para facturación.",
                    )
                    return True
                except Exception as e:
                    show_error_message(
                        ...
                        f"Ocurrió un error al intentar asignar el cliente: {e}",
                    )
                    return False
        return False
        ...
        try:
            ...
            if not hasattr(self.sale_service, "get_sale_by_id"):
                show_error_message(
                    self,
                    "Error de Implementación",
                    "SaleService no tiene el método 'get_sale_by_id'.",
                )
                return
            sale = self.sale_service.get_sale_by_id(self.current_sale_id)
            if not sale:
                show_error_message(
                    self, "Error", f"No se encontró la venta ID {self.current_sale_id}."
                )
                self.invoice_button.setEnabled(False)
                if hasattr(self, "current_sale_id"):
                    delattr(self, "current_sale_id")
            ...
            if not getattr(sale, "customer_id", None):
                if ask_confirmation(
                    self,
                    "Cliente Requerido",
                    "Esta venta no tiene un cliente asignado. ¿Desea asignar uno para facturación?",
                ):
                    if not self._prompt_and_assign_customer_to_sale(
                        self.current_sale_id
                    ):
                        show_info_message(
                            self,
                            "Facturación Cancelada",
                            "No se asignó un cliente. La facturación requiere un cliente.",
                        )
                        return
                    ...
                    if not getattr(sale, "customer_id", None):
                        show_error_message(
                            self,
                            "Error",
                            "No se pudo asignar el cliente. Facturación cancelada.",
                        )
                        return
                else:
                    show_info_message(
                        self,
                        "Facturación Cancelada",
                        "La facturación requiere un cliente.",
                    )
                    return
            ...
            if not hasattr(main_window, "invoicing_service"):
                show_error_message(
                    self, "No se puede acceder al servicio de facturación."
                )
                return

            # Check if the sale already has an invoice
            invoice = main_window.invoicing_service.get_invoice_by_sale_id(
                self.current_sale_id
            )
            if invoice:
                ...
                if response:
                    filename = main_window.invoicing_service.generate_invoice_pdf(
                        invoice.id
                    )
            ...
            invoice = main_window.invoicing_service.create_invoice_from_sale(
                self.current_sale_id
            )
            show_info_message(
                self,
                "Factura Generada",
                f"Factura generada correctamente. Número: {invoice.invoice_number}",
            )
            response = ask_confirmation(
                self, "¿Desea ver la factura generada?", "Ver Factura"
            )
            if response:
                filename = main_window.invoicing_service.generate_invoice_pdf(
                    invoice.id
            ...
            show_error_message(
                ...
                f"Error al generar la factura: {str(e)}",
            )
        else:
            self.invoice_button.setEnabled(False)
            if hasattr(self, "current_sale_id"):
    ...
    def _generate_presupuesto_pdf(self):
        ...
        if not self.sale_item_model.rowCount():
            show_info_message(
                self,
                "Presupuesto Vacío",
                "No hay artículos para generar un presupuesto.",
            )
            return
        ...
        user_name = getattr(
            self.current_user,
            "username",
            getattr(self.current_user, "name", "Usuario Desconocido"),
        )
        ...
        try:
            ...
            if not documents_location:
                ...
                output_dir_base = os.path.join(
                    documents_location, "Eleventa", "Presupuestos"
                )
            ...
            show_error_message(
                self,
                "Error de Directorio",
                f"Error inesperado al configurar directorio de presupuestos: {e}",
            )
            return
        ...
        try:
            ...
            if pdf_file_path:
                show_info_message(
                    ...
                    f"Presupuesto guardado en: {pdf_file_path}",
                )
                ...
                show_error_message(
                    ...
                    "No se pudo generar el archivo PDF del presupuesto (ruta no devuelta).",
                )

        except Exception as e:
            print(
                f"Error generating presupuesto PDF: {e}", flush=True
            )  # Replaced self.logger.error
            import traceback

            traceback.print_exc()  # Print full traceback for debugging
            show_error_message(
                self, "Error al Generar Presupuesto", f"Ocurrió un error: {str(e)}"
            )
