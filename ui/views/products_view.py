...
from ui.models.table_models import (
    ProductTableModel,
    Product,
)  # Assuming Product mock is there too
from ui.dialogs.department_dialog import (
    DepartmentDialog,
    MockProductService_Departments,
)  # Import the dialog
from ui.dialogs.product_dialog import ProductDialog  # Import the Product dialog
from ui.dialogs.update_prices_dialog import (
    UpdatePricesDialog,
)  # Import the Update Prices dialog
from ui.dialogs.unit_management_dialog import (
    UnitManagementDialog,
)  # Import the Unit Management dialog
...
class MockProductService(MockProductService_Departments):  # Inherit department methods
    def __init__(self):
        ...
        self._products = [
            ...
            Product(
                id=3,
                code="SERV01",
                description="Servicio Sin Inventario",
                cost_price=0.0,
                sale_price=50.0,
            ...
            Product(
                id=4,
                code="P003",
                description="Producto B bajo stock",
                cost_price=5.0,
                sale_price=8.0,
                quantity_in_stock=2,
                min_stock=5,
    ...
    def find_product(self, search_term: str = "") -> list[Product]:
        ...
        return [
            ...
            if term in p.code.lower() or term in p.description.lower()
        ]
    ...
    def add_product(self, product_data):  # Simplified
        ...
        if any(p.code.lower() == product_data["code"].lower() for p in self._products):
            raise ValueError(
                f"El código de producto '{product_data['code']}' ya existe."
            )
    ...
    def update_product(self, product_id, product_data):
        ...
        if any(
            p.code.lower() == product_data["code"].lower() and p.id != product_id
            for p in self._products
        ):
            raise ValueError(
                f"Ya existe otro producto con el código '{product_data['code']}'."
            )
...
class ProductsView(QWidget):
    ...
    def _init_ui(self):
        ...
        self.units_button.setIcon(
            QIcon(":/icons/icons/departments.png")
        )  # Using same icon as departments for now
        ...
        self.search_input.addAction(
            QIcon(":/icons/icons/search.png"), QLineEdit.ActionPosition.LeadingPosition
        )
        ...
        self.table_view.horizontalHeader().setStretchLastSection(
            False
        )  # Don't stretch last section automatically
    ...
    def add_new_product(self):
        ...
        if (
            product_dialog.exec()
        ):  # exec() returns 1 (Accepted) if OK was clicked and accept() succeeded
    ...
    def modify_selected_product(self):
        ...
        if selected_product:
            print(
                f"[ProductsView] 'Modify Product' clicked for: {selected_product.code}"
            )
            # Instantiate and exec ProductDialog in 'edit' mode
            product_dialog = ProductDialog(
                self.product_service, product_to_edit=selected_product, parent=self
            )
    ...
    def delete_selected_product(self):
        ...
        print(
            f"[ProductsView] 'Delete Product' clicked for: {selected_product.code}. Asking confirmation..."
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                print(
                    f"[ProductsView] Product ID {selected_product.id} deleted (mock call)."
                )
                self.refresh_products()
                QMessageBox.information(
                    self, "Eliminar Producto", "Producto eliminado correctamente."
                )
            except (
                ValueError
            ) as e:  # Catch potential service errors (e.g., product has stock)
                QMessageBox.warning(
                    self, "Eliminar Producto", f"No se pudo eliminar el producto: {e}"
                )
    ...
    def open_update_prices_dialog(self):
        ...
        try:
            ...
            print(
                "[ProductsView] Update prices dialog closed. Refreshing product list."
            )
            self.refresh_products()
        except Exception as e:
            QMessageBox.critical(
                self,
                "Error",
                f"Error al abrir el diálogo de actualización de precios: {e}",
            )
