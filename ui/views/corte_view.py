...
from infrastructure.reporting.print_utility import (
    print_manager as default_print_manager,
    PrintType,
    PrintDestination,
)
...
class CorteView(QWidget):
    ...
    def __init__(
        self, corte_service: CorteService, user_id: int = None, print_manager=None
    ):
        super().__init__()
    ...
    def _init_ui(self):
        ...
        self.register_filter = FilterDropdown("De la Caja:", [("Caja Principal", 1)])
        ...
        self.cash_in_table.horizontalHeader().setSectionResizeMode(
        ...
        self.cash_out_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.ResizeToContents
        )
    ...
    def _create_section_frame(self, title):
        ...
        frame.setStyleSheet(
            "QFrame { background-color: #f8f8f8; border: 1px solid #ddd; border-radius: 4px; }"
        )
        ...
        try:
            ...
            for payment_type, amount in corte_data["sales_by_payment_type"].items():
                ...
                if payment_type not in self.payment_type_labels:
                    payment_layout = (
                        self.payment_type_labels[PaymentType.EFECTIVO.value]
                        .parent()
                        .layout()
                    )
                    ...
                    payment_layout.addWidget(
                        self.payment_type_labels[payment_type], row, 1
                    )
            ...
            cash_sales = corte_data["sales_by_payment_type"].get(
                "Efectivo", Decimal("0.00")
            )

            # Update cash drawer summary
            self.starting_balance_label.setText(
                f"${corte_data['starting_balance']:.2f}"
            )
        ...
        try:
            if self.current_data is None:
                QMessageBox.warning(
                    self,
                    "Advertencia",
                    "No hay datos de corte cargados. Por favor, genere un reporte primero.",
                )
                return
            ...
            description = (
                f"Cierre de caja del día {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            QMessageBox.information(
                self, "Éxito", "La caja ha sido cerrada exitosamente."
            )
            ...
            self.logger.error(
                f"Error during _on_do_corte: {e}", exc_info=True
            )  # Assuming self.logger exists
            QMessageBox.critical(
                self, "Error", f"Ocurrió un error al cerrar la caja: {str(e)}"
            )
    ...
    def _print_report(self):
        ...
        if self.current_data is None:
            QMessageBox.warning(
                self, "Advertencia", "No hay datos para imprimir el reporte."
            )
            return
        try:
            start_time, end_time = self.period_filter.get_period_range()
            ...
            cash_sales = self.current_data["sales_by_payment_type"].get(
                "Efectivo", Decimal("0.00")
            )
            expected_cash = (
                self.current_data["starting_balance"]
            ...
            print_data = {
                ...
                "actual_cash": Decimal(str(self.actual_cash_input.value())),
                "difference": Decimal(str(self.actual_cash_input.value()))
                - expected_cash,
            ...
            if not result:
                QMessageBox.warning(
                    self, "Error", "Ocurrió un error al generar el reporte."
                )
        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Error al imprimir el reporte: {str(e)}"
            )
