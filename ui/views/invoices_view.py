...
class InvoicesView(QWidget):
        ...
        try:
            ...
            for invoice in invoices:
                ...
                if (
                    ...
                    and search_text in invoice.invoice_number.lower()
                ):
                ...
                if (
                    ...
                    and "name" in invoice.customer_details
                    and search_text in invoice.customer_details["name"].lower()
                ):
                    filtered_invoices.append(invoice)
                    continue
    ...
    def view_invoice(self):
        ...
        try:
            ...
            if os.path.exists(filename):
                ...
                show_error_message(
                    self, "Error", f"El archivo de factura no existe: {filename}"
                )

        except Exception as e:
            show_error_message(
                self, "Error", f"Error al generar el PDF de la factura: {str(e)}"
            )
    ...
    def print_invoice(self):
        ...
        try:
            ...
            show_info_message(
                self,
                "Factura Generada",
                "El archivo de factura se ha generado correctamente.\n\n"
                "Se abrirá ahora para que pueda imprimirlo desde su visor de PDF.",
            )
            ...
            show_error_message(
                self, "Error", f"Error al generar el PDF para impresión: {str(e)}"
            )
    ...
    def save_invoice_pdf(self):
        ...
        if invoice_id is None:
            show_error_message(
                self,
                "Selección Requerida",
                "Por favor seleccione una factura para guardar como PDF.",
            )
            return
        ...
        try:
            ...
            if not invoice:
                show_error_message(
                    self, "Error", "No se pudo encontrar la factura seleccionada."
                )
                return
            ...
            filename, _ = QFileDialog.getSaveFileName(
                self, "Guardar Factura como PDF", suggested_name, "Archivos PDF (*.pdf)"
            )
            ...
            show_info_message(
                self, "Éxito", f"La factura se ha guardado como {filename}"
            )

        except Exception as e:
            show_error_message(
                self, "Error", f"Error al guardar el PDF de la factura: {str(e)}"
            )
    ...
    def cancel_invoice(self, row):
        ...
        show_info_message(
            self,
            "No Implementado",
            "La funcionalidad para anular facturas no está implementada aún.",
        )
