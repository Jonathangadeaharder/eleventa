...
class CashDrawerView(QWidget):
    ...
    def _refresh_data(self):
        ...
        self.actual_balance_label.setText(
            f"${current_balance:,.2f}"
        )  # Reuse formatted current_balance
    ...
    def _handle_open_drawer(self):
        ...
        if is_open:
            ...
            if close_dialog.exec():
                ...
                if (
                    close_dialog.entry
                ):  # Check if the dialog successfully created an entry
    ...
    def _handle_add_cash(self):
        ...
        if not summary.get("is_open", False):
            QMessageBox.warning(
                self, "Error", "La caja debe estar abierta para agregar efectivo."
            )
            return

        # Use the correct dialog class name (alias), pass service/user_id, and parent
        dialog = CashMovementDialog(
            self.service, self.user_id, is_adding=True, parent=self
        )
        if dialog.exec():
            # The dialog now handles the service call and message boxes internally
            # We just need to refresh the view if it succeeded
    ...
    def _handle_remove_cash(self):
        ...
        if not summary.get("is_open", False):
            QMessageBox.warning(
                self, "Error", "La caja debe estar abierta para retirar efectivo."
            )
            return

        # Use the correct dialog class name (alias), pass service/user_id, and parent
        dialog = CashMovementDialog(
            self.service, self.user_id, is_adding=False, parent=self
        )
        if dialog.exec():
            # The dialog now handles the service call and message boxes internally
            # We just need to refresh the view if it succeeded
    ...
    def _print_report(self):
        ...
        if not drawer_summary.get("is_open", False):
            QMessageBox.warning(
                self, "Error", "La caja debe estar abierta para imprimir el reporte."
            )
            return
...
__all__ = ["parse_currency_input", "calculate_difference", "CashDrawerView"]
