...
class ImportExportDialog(QDialog):
    ...
    def _import_from_excel(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Importar desde Excel", "", "Archivos Excel (*.xlsx)"
        )
    ...
    def _import_from_csv(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Importar desde CSV", "", "Archivos CSV (*.csv)"
        )
    ...
    def _restore_backup(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Restaurar Respaldo", "", "Archivos JSON (*.json)"
        )
    ...
    def _start_operation(self, operation_type, file_path):
        ...
        if self.worker and self.worker.isRunning():
            QMessageBox.warning(
                self, "Operación en Progreso", "Ya hay una operación en progreso."
            )
            return
