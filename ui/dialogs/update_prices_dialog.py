...
class UpdatePricesDialog(QDialog):
    def __init__(self, product_service: ProductService, parent=None):
        ...
        self.percentage_input.setPlaceholderText(
            "Ej: 10 para aumentar, -5 para disminuir"
        )
    ...
    def get_percentage(self) -> Decimal | None:
        text = self.percentage_input.text().replace(
            ",", "."
        )  # Allow comma as decimal separator
        if not text:
            QMessageBox.warning(
                self, "Entrada Inválida", "Por favor ingrese un porcentaje."
            )
            return None
        try:
            ...
            if val <= Decimal("-100"):
                QMessageBox.warning(
                    self, "Entrada Inválida", "El porcentaje debe ser mayor que -100%."
                )
                return None
            return val
            ...
            QMessageBox.warning(
                self,
                "Entrada Inválida",
                "Porcentaje inválido. Use números (ej: 10.5 o -5).",
            )
            return None
    ...
    def accept(self):
        ...
        confirm_msg = (
            f"¿Está seguro que desea actualizar los precios en un {percentage}% "
        )
        if department_id:
            confirm_msg += f"para el departamento '{department_name}'?"
        else:
            confirm_msg += "para TODOS los productos?"
        ...
        if reply == QMessageBox.Yes:
            try:
                updated_count = self.product_service.update_prices_by_percentage(
                    percentage, department_id
                )
                QMessageBox.information(
                    self,
                    "Éxito",
                    f"Se actualizaron los precios de {updated_count} producto(s).",
                )
                super().accept()
                ...
                QMessageBox.critical(
                    self, "Error", f"Ocurrió un error al actualizar los precios: {e}"
                )
...
if __name__ == "__main__":
    ...
    class MockDepartmentRepository:
        def get_all(self):
            return [
                Department(
                    id=1, name="Electrónica", description="Artículos electrónicos"
                ),
    ...
    class MockProductRepository:
        ...
        def update(self, product):
            print(
                f"Mock: updating product {product.id} with new price {product.sell_price}"
            )
        ...
        def get_low_stock(self, limit):
            return []

        def update_stock(self, prod_id, qty, cost):
            return None
