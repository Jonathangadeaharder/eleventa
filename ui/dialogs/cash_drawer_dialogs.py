...
class OpenCashDrawerDialog(QDialog):
    ...
    def __init__(
        self, cash_drawer_service: CashDrawerService, user_id: int, parent=None
    ):
        try:
    ...
    def accept(self):
        ...
        try:
            ...
            description = (
                self.description_field.toPlainText() or "Apertura inicial de caja"
            )
...
class AddRemoveCashDialog(QDialog):
    ...
    def accept(self):
        ...
        try:
            ...
            if not description:
                QMessageBox.warning(
                    self, "Error", "Por favor ingrese una descripción del movimiento."
                )
                return
            ...
            if self.is_adding:
                self.entry = self.cash_drawer_service.add_cash(
                    amount=amount, description=description, user_id=self.user_id
                )
                operation = "agregado"
            else:
            ...
            QMessageBox.critical(
                self, "Error", f"Error al procesar la operación: {str(e)}"
            )
...
class CashDrawerHistoryDialog(QDialog):
    ...
    def __init__(
        self, cash_drawer_service: CashDrawerService, parent=None, test_mode=False
    ):
        try:
            super().__init__(parent)
            self.cash_drawer_service = cash_drawer_service
...
class CloseCashDrawerDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.actual_amount_field.setRange(
            0, 1000000
        )  # Allow 0 for cases where drawer might be empty
    ...
    def accept(self):
        ...
        try:
            ...
            description = (
                ...
                or f"Cierre de caja - {date.today().strftime('%Y-%m-%d')}"
            )
