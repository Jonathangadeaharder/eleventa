    ...
    def _update_result_label(self):
        ...
        if new_stock < 0:
            self.result_label.setText(
                f"ERROR: ¡El ajuste resultaría en stock negativo! ({new_stock:.2f})"
            )
            self.result_label.setStyleSheet("color: red; font-weight: bold;")
        else:
            self.result_label.setText(
                f"{current_stock:.2f} {sign} {adjustment:.2f} = {new_stock:.2f}"
            )
    ...
    def accept(self):
        ...
        if quantity <= 0:
            show_error_message(
                self,
                "Cantidad Inválida",
                "La cantidad a ajustar debe ser mayor que cero.",
            )
        ...
        if not reason:
            show_error_message(
                self, "Motivo Requerido", "Por favor, ingrese el motivo del ajuste."
            )
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error Inesperado",
                f"Ocurrió un error al ajustar el inventario: {e}",
            )
