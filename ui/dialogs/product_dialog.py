...
from PySide6.QtWidgets import (
    ...
    QWidget,
    QFrame,
    QGroupBox,  # Added QGroupBox, QSpacerItem, QSizePolicy
)
...
class ProductDialog(QDialog):
    ...
    def __init__(
        ...
        self.setWindowTitle(
            "Modificar Producto" if self.is_edit_mode else "Agregar Producto"
        )
    ...
    def _init_ui(self):
        ...
        container.setStyleSheet(
            "QFrame { background-color: #f9f9f9; border-radius: 4px; }"
        )
        ...
        self.stock_input.setRange(
            -99999.99, 999999.99
        )  # Allow negative temporarily if needed
        ...
        self.save_button = QPushButton(
            "Guardar" if self.is_edit_mode else "Crear Producto"
        )
    ...
    def _load_departments(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Error",
                "El servicio de productos no tiene el método 'get_all_departments'.",
            )
        except Exception as e:
            QMessageBox.warning(
                self, "Error", f"No se pudieron cargar los departamentos:\n{e}"
            )
    ...
    def _load_units(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Error", f"No se pudieron cargar las unidades:\n{e}"
            )
    ...
    def _populate_form(self):
        ...
        if product.department_id:
            ...
            if index >= 0:
                ...
                print(
                    f"Warning: Department ID {product.department_id} not found for product {product.code}"
                )
    ...
    def accept(self):
        ...
        try:
            ...
            department_id = (
                self.department_combo.itemData(dept_index) if dept_index > 0 else None
            )
            ...
            product_obj = Product(
                ...
                quantity_in_stock=(
                    ...
                    if not self.is_edit_mode and uses_inventory
                        ...
                        self.product_to_edit.quantity_in_stock
                        if self.is_edit_mode
                        else 0.0
                    )
                ),
