...
class ConcreteRepository(RepositoryBase[TestDomainModel]):
    ...
    def _entity_to_domain(self, entity):
        return TestDomainModel(id=entity.id, name=entity.name)
    
    def _domain_to_entity(self, domain_model):
        return TestEntity(id=domain_model.id, name=domain_model.name)
    
    def get_by_id(self, id):
        # Instead of using TestEntity.id, which causes errors as TestEntity isn't an SQLAlchemy model,
        # we'll just mock the entire query chain
        query = self.session.query.return_value
        filter_query = query.filter.return_value
        mock_entity = TestEntity(id=id, name=f"Entity {id}")
        filter_query.first.return_value = mock_entity
        
        # Use the mock chain directly
        entity = self.session.query(TestEntity).filter().first()
        return self._entity_to_domain(entity) if entity else None
    
    def get_all(self):
        # Simulate querying with session
        query = self.session.query.return_value
        query.all.return_value = [TestEntity(id=1, name="Entity 1"), TestEntity(id=2, name="Entity 2")]
        entities = self.session.query(TestEntity).all()
...
def test_entity_to_domain():
    ...
    # Assert
    assert isinstance(domain_model, TestDomainModel)
...
def test_domain_to_entity():
    # Arrange
    repo = ConcreteRepository()
    domain_model = TestDomainModel(id=1, name="Test")
...
def test_get_by_id(repository, mock_session):
    # Arrange
    test_id = 1
    mock_entity = TestEntity(id=test_id, name=f"Entity {test_id}")
...
def test_get_all(repository, mock_session):
    # Arrange
    mock_entities = [TestEntity(id=1, name="Entity 1"), TestEntity(id=2, name="Entity 2")]
    query = mock_session.query.return_value
    ...
    assert all(isinstance(result, TestDomainModel) for result in results)
...
def test_add(repository, mock_session):
    # Arrange
    domain_model = TestDomainModel(name="New Entity")
...
def test_update(repository, mock_session):
    # Arrange
    domain_model = TestDomainModel(id=1, name="Updated Entity")
...
def test_delete(repository, mock_session):
    ...
    def query_side_effect(entity_class):
        # This will be called when session.query(TestEntity) is executed
        return query
    ...
    # Assert
    mock_session.query.assert_called_once_with(TestEntity)
    query.filter.assert_called_once()
    filter_query.first.assert_called_once()
    ...
    assert isinstance(deleted_entity, TestEntity)
