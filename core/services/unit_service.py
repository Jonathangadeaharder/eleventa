...
class UnitService(ServiceBase):
    ...
    def _validate_unit(
        ...
        existing_unit_id: Optional[int] = None,
    ):
    ...
    def add_unit(self, unit: Unit) -> Unit:
        ...
        with unit_of_work() as uow:
            try:
                ...
                self.logger.info(
                    f"Unit added successfully: {added_unit.name} (ID: {added_unit.id})"
                )
                return added_unit
    ...
    def update_unit(self, unit: Unit) -> Unit:
        ...
        with unit_of_work() as uow:
            try:
                ...
                self.logger.info(
                    f"Unit updated successfully: {updated_unit.name} (ID: {updated_unit.id})"
                )
                return updated_unit
        ...
        with unit_of_work() as uow:
            try:
                ...
                if success:
                    uow.commit()
                    self.logger.info(
                        f"Unit deleted successfully: {existing.name} (ID: {unit_id})"
                    )
