            ...
            with unit_of_work() as uow:
                ...
                for product in products:
                    data.append({
                        ...
                        'Precio Costo': float(product.cost_price) if product.cost_price else 0,
                        'Precio Venta': float(product.sell_price) if product.sell_price else 0,
                        'Stock': float(product.quantity_in_stock) if product.quantity_in_stock else 0,
                        'Stock Mínimo': float(product.min_stock) if product.min_stock else 0,
                        'Unidad': product.unit if hasattr(product, 'unit') else 'Unidad',
                        'Usa Inventario': 'Sí' if product.uses_inventory else 'No',
                        'Departamento': departments.get(product.department_id, '') if product.department_id else ''
                    })
                
                df = pd.DataFrame(data)
                df.to_excel(file_path, index=False, engine='openpyxl')
                
                self.logger.info(f"Exported {len(products)} products to Excel: {file_path}")
    ...
    def export_products_to_csv(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            with unit_of_work() as uow:
                ...
                self.logger.info(f"Exported {len(products)} products to CSV: {file_path}")
            ...
            with unit_of_work() as uow:
                ...
                for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), 2):
                    ...
                    try:
                        ...
                        usa_inventario = str(row[7]).strip().lower() in ['si', 'sí', 'yes', 'true', '1'] if row[7] else True
                        departamento_nombre = str(row[8]).strip() if row[8] else ""
                        
                        if not codigo or not descripcion:
                            results["errors"].append(f"Fila {row_num}: Código y descripción son obligatorios")
                            results["skipped"] += 1
                            continue
                ...
                results["message"] = f"Importación completada: {results['imported']} nuevos, {results['updated']} actualizados, {results['skipped']} omitidos"
                ...
                with open(file_path, 'r', encoding='utf-8') as csvfile:
                    ...
                    for row_num, row in enumerate(reader, 2):
                        ...
                        try:
                            ...
                            usa_inventario = row.get('usa_inventario', 'Si').strip().lower() in ['si', 'sí', 'yes', 'true', '1']
    ...
    def create_backup(self, backup_path: str) -> Dict[str, Any]:
        ...
        try:
            with unit_of_work() as uow:
                    ...
                    "products": [
                        {
                            ...
                            "sale_price": float(product.sell_price) if product.sell_price else 0,
                            "quantity_in_stock": float(product.quantity_in_stock) if product.quantity_in_stock else 0,
                ...
                self.logger.info(f"Created backup with {len(products)} products, {len(departments)} departments")
                ...
                if "departments" in backup_data:
                    for dept_data in backup_data["departments"]:
                        try:
                            ...
                            results["errors"].append(f"Error restaurando departamento '{dept_data.get('name', 'N/A')}': {e}")
                    ...
                    for product_data in backup_data["products"]:
                        try:
                            ...
                            if product_data.get("department_id"):
                                dept = uow.departments.get_by_id(product_data["department_id"])
                                if dept:
                                    department_id = dept.id
                            ...
                            product_obj_data = {
                                ...
                                "sell_price": Decimal(str(product_data.get("sale_price", 0))),
                                "quantity_in_stock": Decimal(str(product_data.get("quantity_in_stock", 0))),
                                "min_stock": Decimal(str(product_data.get("min_stock", 0))),
                            ...
                            results["errors"].append(f"Error restaurando producto '{product_data.get('code', 'N/A')}': {e}")
                ...
                if "customers" in backup_data:
                    for customer_data in backup_data["customers"]:
                        try:
                            ...
                            results["errors"].append(f"Error restaurando cliente '{customer_data.get('name', 'N/A')}': {e}")
                
                self.logger.info(f"Restored {results['products_restored']} products, {results['departments_restored']} departments")
                
                results["message"] = f"Restauración completada: {results['products_restored']} productos, {results['departments_restored']} departamentos, {results['customers_restored']} clientes."
